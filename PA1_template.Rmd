...
```{r, echo=FALSE}
plot(steps, type="h", xlab="Day", ylab="Steps", main="Total Steps Per Day")   # plot the histogram
```
...
```{r, echo=FALSE}
daily <- aggregate(steps~interval, data=act_clean, FUN="mean")
plot(daily, type="l", xlab="5 Minute Interval", ylab="Steps", main="Average Number Of Steps Per 5 Minute Interval")
```
...
```{r, echo=FALSE}
steps.new <- aggregate(steps~date, data=act.new, FUN=sum)
plot(steps.new, type="h", xlab="Day", ylab="Steps", main="Total Steps Per Day") # plot the histogram
```
...
```
 ...
 - The panel plot blow confronts the activity pattern on workdays with weekdays. On each panel it shows the average steps taken throughout the measured 5 minute intervals (5 o'clock appears as 500, 10 o'clock as 1000). It shows that there is a morning rush (possibly to work?) on weekdays, a mostly calm activity throughout the day and a small peak around 7 pm (possibly going home). On weekends the activity is more balanced throughout the day and it starts slightly later around 7 am but lasts longer till after 8 pm.
