...
```{r, echo=FALSE}
hist(steps$steps, breaks=length(steps$steps), col="darkolivegreen4")      # plot the histogram
```
...
hist(steps.new$steps, breaks=length(steps.new$steps), col="darkolivegreen3")
```
